...
from nn.models import TextEmbedder, get_device, MultiLabelClassifier
...
def process_labels(labels_str, label_map, num_classes):
    ...
    for label in labels_str.split('|'):
        ...
        if label not in label_map:
            raise ValueError(f"Label '{label}' not found in label mapping. Please check your training data or labels.csv.")
...
def train_epoch(model, data_loader, loss_fn, optimizer, device, epoch, epochs):
    model.train()
    losses = []
    progress_bar = tqdm(data_loader, desc=f'Epoch {epoch}/{epochs} [Train]', leave=False)
...
for epoch in range(EPOCHS):
    ...
    train_loss = train_epoch(model, train_dataloader, loss_fn, optimizer, device, epoch + 1, EPOCHS)
    val_loss, val_f1_micro, val_f1_macro = eval_model(model, val_dataloader, loss_fn, device)
    ...
    print(f"Epoch {epoch + 1} Summary: Train loss: {train_loss:.4f}, Val loss: {val_loss:.4f}, Val F1 (Micro): {val_f1_micro:.4f}, Val F1 (Macro): {val_f1_macro:.4f}")
