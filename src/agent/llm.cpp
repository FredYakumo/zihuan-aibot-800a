...
namespace agent {
        ...
        std::string gen_common_prompt(const bot_adapter::Profile &bot_profile, const bot_adapter::BotAdapter &adapter,
            ...
            if (const auto &group_sender = bot_adapter::try_group_sender(sender); group_sender.has_value()) {
                ...
                std::string bot_perm = get_permission_chs(
                    adapter.get_group(group_sender->get().group.id).group_info.bot_in_group_permission);
            ...
            for (auto it = group_msg_list.crbegin(); it != group_msg_list.crend(); ++it) {
                ...
                if (msg.sender_id == target_id) {
                    ...
                    target_msgs.insert(target_msgs.begin(),
                                       fmt::format("[{}]'{}': '{}'", time_point_to_db_str(msg.send_time),
        ...
        msg_list_to_json(const std::deque<ChatMessage> &msg_list,
            ...
            while (one_chat_session.rbegin()->tool_calls) {
                ...
                for (const auto &func_calls : *llm_res.tool_calls) {
                    ...
                    if (func_calls.name == "search_info") {
                        ...
                        first_replay.emplace_back(context.adapter.get_bot_profile().id,
                        ...
                        if (urls.empty()) {
                            ...
                            context.adapter.send_long_plain_text_reply(
                                *context.event->sender_ptr,
                                ltrim(rtrim(llm_res.content)).empty() ? "等我看看这个链接哦..." : llm_res.content,
                                true);
                        }
                        ...
                        if (const auto &group_sender = bot_adapter::try_group_sender(*context.event->sender_ptr);
                            ...
                            if (target_id_opt.has_value()) {
                                ...
                                if (target_name.has_value() && !target_name->empty()) {
                                    ...
                                    std::vector<qq_id_t> target_id_list =
                                        context.adapter.group_member_name_embedding_map
                                            .find(group_sender->get().group.id)
                                            ->get()
                                    ...
                                    if (msg_list.empty()) {
                                        content =
                                            fmt::format("在'{}'群里还没有聊天记录哦", group_sender->get().group.name);
                                    } else {
                                        std::string text;
                            ...
                            const auto &msg_list = g_person_message_storage.get_individual_last_msg_list(
                                context.event->sender_ptr->id, 1000);
                            if (msg_list.empty()) {
                        ...
                        tool_call_msg = ChatMessage(
                            ROLE_TOOL, fmt::format("可用功能/函数列表: {}", bot_cmd::get_available_commands()),
                            func_calls.id);
                    } else {
...
...
            ...
            try {
                ...
                std::string result =
                    std::string(ltrim(json["choices"][0]["message"]["content"].get<std::string_view>()));
