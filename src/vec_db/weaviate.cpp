...
namespace vec_db {
        ...
        try {
            ...
            if (lac_model) {
                ...
                for (const auto& item : lac_result) {
                    ...
                    // vd, vn: 动词衍生词
                    // a: 形容词
                    if (item.tag.starts_with("n") || 
                        item.tag.starts_with("v") ||
                        item.tag.starts_with("a") ||
                        item.tag.starts_with("PER") ||
                        item.tag.starts_with("LOC") ||
                        item.tag.starts_with("ORG") ||
    ...
    std::vector<DBKnowledge> query_knowledge_from_vec_db(const std::string_view query, float certainty_threshold,
        ...
        cpr::Response response = cpr::Post(
            ...
            cpr::Body{
                nlohmann::json{{"query", graphql_query("AIBot_knowledge", emb, keyword_query, certainty_threshold, top_k)}}.dump(),
            },
                    ...
                    if (auto knowledge = get_optional(get.value(), "AIBot_knowledge"); knowledge.has_value()) {
                        ...
                        for (const auto &item : knowledge.value()) {
                            ...
                            if (auto additional = get_optional(item, "_additional"); additional.has_value()) {
                                ...
                                try {
                                    if (additional.value().contains("certainty") && !additional.value()["certainty"].is_null()) {
                                ...
                                try {
                                    if (additional.value().contains("score") && !additional.value()["score"].is_null()) {
                                ...
                                if (hybrid_score >= certainty_threshold) {
                                    ...
                                    spdlog::info(
                                        "Found knowledge (filtered): filter={}, content={}, keywords={}, creator={}, time={}, certainty={}, hybrid_score={}",
                                        db_knowledge.knowledge_class_filter,
                                        db_knowledge.content.substr(0, 50) + (db_knowledge.content.length() > 50 ? "..." : ""),
                                    ...
                                    spdlog::debug("Skipped low-score result: {}, score={}, threshold={}", 
                                        db_knowledge.content.substr(0, 30) + (db_knowledge.content.length() > 30 ? "..." : ""),
                                        hybrid_score,
                                        certainty_threshold);
                                }
                            } else {
                                ...
                                spdlog::info(
                                    "Found knowledge (unscored): filter={}, content={}, keywords={}, creator={}, time={}, certainty={}",
                                    db_knowledge.knowledge_class_filter,
                                    db_knowledge.content.substr(0, 50) + (db_knowledge.content.length() > 50 ? "..." : ""),
                                    wheel::join_str(std::cbegin(db_knowledge.keyword), std::cend(db_knowledge.keyword), ","),
                                    db_knowledge.creator_name,
                        ...
                        // 查询完成后显示统计信息
                        spdlog::info("查询知识完成: 原始结果 {} 条, 过滤后 {} 条, 耗时: {} ms", 
                                    total_found, 
                                    results.size(),
                                    duration.count());
                    }
