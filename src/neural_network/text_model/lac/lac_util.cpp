...
namespace lac {
     ...
     */
    RVAL split_tokens(const std::string &line, const std::string &pattern, std::vector<std::string> &tokens) {
        if ("" == line || "" == pattern) {
     ...
     */
    RVAL load_word2id_dict(const std::string &filepath, std::unordered_map<std::string, int64_t> &kv_dict) {
     ...
     */
    RVAL load_q2b_dict(const std::string &filepath, std::unordered_map<std::string, std::string> &kv_dict) {
        std::ifstream fin(filepath);
        if (!fin) {
     ...
     */
    RVAL load_id2label_dict(const std::string &filepath, std::unordered_map<int64_t, std::string> &kv_dict) {
        std::ifstream fin(filepath);
        if (!fin) {
    ...
    int get_next_gb18030(const char *str) {
        ...
        if (one >= 0x81 && one <= 0xfe && two >= 0x30 && two <= 0x39 && three >= 0x81 && three <= 0xfe &&
            four >= 0x30 && four <= 0x39) {
            // 4字节-扩展汉字、生僻字
            return 4;
        }
