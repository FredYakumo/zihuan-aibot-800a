// This file now serves as an aggregator for the split model implementations
// All actual implementations have been moved to separate files under text_model/

#include "neural_network/text_model/text_embedding_model.cpp"
#include "neural_network/text_model/text_embedding_with_mean_pooling_model.cpp"
#include "neural_network/text_model/tokenizer_wrapper.cpp"
    ...
    emb_vec_t TextEmbeddingWithMeanPoolingModel::embed(const token_id_list_t &token_ids,
        ...
        torch::Tensor input_ids_tensor =
            torch::tensor(std::vector<int32_t>(token_ids.begin(), token_end), tensor_options).unsqueeze(0);
        torch::Tensor attention_mask_tensor =
            torch::tensor(std::vector<int32_t>(attention_mask.begin(), mask_end), tensor_options).unsqueeze(0);
    ...
    emb_mat_t TextEmbeddingWithMeanPoolingModel::embed(const std::vector<token_id_list_t> &token_ids,
        ...
        for (size_t start_idx = 0; start_idx < total_size; start_idx += max_batch_size) {
            ...
            torch::Tensor embedding_tensor;
            {
                ...
                torch::jit::IValue output = m_module.forward(inputs);
                embedding_tensor = output.toTensor();

                // Convert to CPU immediately to free GPU memory
                embedding_tensor = embedding_tensor.to(torch::kCPU);
            }
            ...
            auto embedding_accessor = embedding_tensor.accessor<float, 2>();
            const auto hidden_size = embedding_tensor.size(1);

            // Add embeddings to result
            for (size_t i = 0; i < current_batch_size; ++i) {
                emb_vec_t embedding;
                embedding.reserve(hidden_size);
                const float *embedding_data = &embedding_accessor[i][0];
                embedding.assign(embedding_data, embedding_data + hidden_size);
