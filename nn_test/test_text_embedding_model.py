...
from nn.models import CosineSimilarityModel, TextEmbedder, get_device
...
def test_batch_text_embedding():
    ...
    print(
        f"Generated {batch_embeddings.shape[0]} embeddings with dimension {batch_embeddings.shape[1]}"
    )
...
def test_cosine_similarity_model(
    target_embedding: torch.Tensor, batch_embeddings: torch.Tensor
):
    ...
    # 验证结果
    assert len(similarity_scores_np) == len(
        batch_text
    ), "Similarity scores count mismatch"
    ...
    # 验证"张三是吕四的什么人"与自身的相似度最高
    max_score_idx = np.argmax(similarity_scores_np)
    assert (
        batch_text[max_score_idx] == "张三是吕四的什么人"
    ), f"Expected '张三是吕四的什么人' to have highest similarity, got '{batch_text[max_score_idx]}'"
...
def test_token_level_embeddings():
    ...
    for text in batch_text:
        ...
        individual_token_embeddings.append(
            token_embeddings[0]
    ...
    start_time = time.time()
    batch_token_embeddings = token_embedder.embed(
        batch_text
...
def main():
    ...
    print(
        "This test replicates the C++ unit test functionality to help debug potential issues."
    )
    ...
    try:
        ...
        pytorch_scores = test_cosine_similarity_model(
            target_embedding, batch_embeddings
        )
