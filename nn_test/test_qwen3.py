...
def create_system_prompt(high_reward_content: str = "讨厌华为手机,因为爱国营销太傻逼了") -> str:
    """创建系统提示词"""
...
def prepare_messages(user_prompt: str, system_prompt: str = None) -> List[Dict[str, str]]:
    """准备对话消息"""
...
def generate_response(model, tokenizer, messages: List[Dict[str, str]], device, 
                     max_new_tokens: int = 32768, enable_thinking: bool = False) -> Dict[str, str]:
    """生成模型响应"""
...
def parse_json_response(response: str, thinking_content: str = "") -> Dict[str, Any]:
    ...
    if thinking_content:
        ...
        if result:
            return {"source": "thinking", "data": result, "thinking_content": thinking_content}
...
def display_results(result: Dict[str, str], parsed_json: Dict[str, Any]):
    ...
    if "error" not in parsed_json:
        print(f"✅ JSON parsed successfully from: {parsed_json.get('source', 'unknown')}")
        print("📋 Structured output:")
        print(json.dumps(parsed_json.get('data', parsed_json), indent=2, ensure_ascii=False))
