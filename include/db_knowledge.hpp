...
struct DBKnowledge {
    ...
    DBKnowledge(const std::string_view content, const std::string_view creator_name, const std::string_view create_dt, const std::vector<std::string> &keywords, float certainty):
        content(content), creator_name(creator_name), create_dt(create_dt), keywords(keywords), certainty(certainty) {}
};
